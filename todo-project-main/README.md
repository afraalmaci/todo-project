...
A full-stack to-do app with per-user accounts: React on the frontend, a Spring Boot REST API on the backend, PostgreSQL for storage, and JWT-based authentication so every user only ever sees their own todos.
...
- Optional due date and free-form tags per todo, with filtering by tag
...
| | |
|---|---|
| Frontend | React 19, React Router, Axios/fetch, Yup validation |
